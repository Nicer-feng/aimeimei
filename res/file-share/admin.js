...
(async()=>{
  ...
  const office=window.CloudOffice;
  ...
  $('refreshUpdate').onclick=()=>{
    ...
    if(office.isOpen()){toast('请先关闭在线编辑器，再刷新更新');return;}
  ...
  async function refreshFiles(automatic){
    ...
    if(uploading||office.isOpen()||$('dialog').open||$('fileContextMenu')){if(!automatic)toast('请先完成当前上传或操作');return;}
