...
(async()=>{
  ...
  async function navigate(next){section=sections.some(s=>s[0]===next)?next:'overview';page=1;history.replaceState(null,'','#'+section);await render();}
  ...
  function logTable(logs){if(!logs.length)return empty('暂无访问记录','activity');return `<div class="table-wrap"><table><thead><tr><th>时间</th><th>访问者</th><th>浏览器 / 系统</th><th>行为</th><th>文件 / 分享</th><th>结果</th></tr></thead><tbody>${logs.map(l=>`<tr><td>${date(l.created_at)}</td><td>${esc([l.province,l.city].filter(Boolean).join(' '))}<div>${esc(l.ip)}</div></td><td>${esc(l.browser)} / ${esc(l.os)}<br><small>${esc(l.device)}</small></td><td>${esc(actions[l.action]||l.action)}</td><td>${esc(l.filename||l.title)}</td><td>${l.success?'成功':'<span class="error">未通过</span>'}</td></tr>`).join('')}</tbody></table></div>`;}
  ...
  async function render(){
    ...
    if(section==='overview'){
      ...
      html=(section==='files'?`<div id="dropzone" class="dropzone" role="button" tabindex="0">${icon('cloud-upload')} 拖拽文件到这里，或点击选择文件 <small>· 图片、视频、文档及普通文件</small></div>`:'')+`<div class="toolbar"><input id="searchFiles" placeholder="搜索文件名" aria-label="搜索文件名" value="${esc(filters.search)}"><select id="typeFilter" aria-label="文件类型"><option value="">全部类型</option>${['IMAGE','VIDEO','AUDIO','PDF','WORD','EXCEL','PPT','ARCHIVE','TEXT','OTHER'].map(t=>`<option ${filters.type===t?'selected':''}>${t}</option>`).join('')}</select><select id="sortFilter" aria-label="上传时间排序"><option value="newest" ${filters.sort==='newest'?'selected':''}>最新上传</option><option value="oldest" ${filters.sort==='oldest'?'selected':''}>最早上传</option></select><button id="searchButton">搜索</button></div>`;
  ...
  function bindContent(){
    ...
    if($('settingsForm'))$('settingsForm').onsubmit=async e=>{e.preventDefault();const data=Object.fromEntries(new FormData(e.target));try{await api(base+'/settings',{display_name:data.display_name,max_upload_bytes:Math.round(Number(data.max_gb)*1024**3)});toast('设置已保存');}catch(error){toast(error.message);}};
  }
  ...
  async function uploadFiles(files){
    if(uploading)return toast('请等待当前上传队列完成');if(!files.length)return;uploading=true;$('uploads').hidden=false;
