...
(async()=>{
  ...
  const office=window.CloudOffice,pdf=window.CloudPdfEditor;
  office.configure({base,api,toast,esc,size,refreshIcons,onChanged:async()=>{if(section==='files')await render(true);}});
  pdf.configure({base,api,toast,esc,size,refreshIcons,onChanged:async()=>{if(section==='files')await render(true);}});
  ...
  $('refreshUpdate').onclick=()=>{
    ...
    if(office.isOpen()||pdf.isOpen()){toast('请先关闭编辑器，再刷新更新');return;}
  ...
  async function render(quiet=false){
    ...
    if(section==='platform'){html=await CloudPlatform.render(page);
      ...
      if(section==='files'&&office.hint())html+=`<p class="muted file-hint">${esc(office.hint())}</p>`;
      html+=section==='files'&&fileView!=='list'?iconFiles(d.items):d.items.length?`<div class="table-wrap"><table><thead><tr>${section==='files'?'<th><input type="checkbox" id="selectAll" aria-label="选择本页全部文件"></th>':''}<th>文件名</th><th>大小</th><th>上传时间</th><th title="仅统计已保存的文档和 PDF 版本，草稿自动同步不计">最近保存版本</th><th>分享</th><th>访问 / 下载</th><th>操作</th></tr></thead><tbody>${d.items.map(f=>`<tr>${section==='files'?`<td><input type="checkbox" data-select="${f.id}" aria-label="选择 ${esc(f.filename)}" ${selected.has(f.id)?'checked':''}></td>`:''}<td><div class="file-cell">${fileIcon(f)}<span class="filename">${esc(f.filename)}<br><small>${f.file_type}</small></span></div></td><td>${size(f.size)}</td><td>${date(f.created_at)}</td><td class="file-time">${f.last_edited_at?date(f.last_edited_at):'—'}</td><td>${f.active_share_count?'分享中':'未分享'}<br><small>累计 ${f.share_count} 次</small></td><td>${f.view_count} / ${f.download_count}</td><td><div class="table-actions">${section==='trash'?`<button data-file="${f.id}" data-action="restore">恢复</button><button data-file="${f.id}" data-action="purge" class="danger">永久删除</button>`:`<button data-file="${f.id}" data-action="preview">预览</button>${office.canEdit(f)?`<button data-file="${f.id}" data-action="office-edit">在线编辑</button><button data-file="${f.id}" data-action="office-versions">版本</button>`:''}${pdf.canEdit(f)?`<button data-file="${f.id}" data-action="pdf-edit">整理页面</button><button data-file="${f.id}" data-action="pdf-versions">版本</button>`:''}<button data-single-share="${f.id}">分享</button><button data-file="${f.id}" data-action="rename">重命名</button><button data-file="${f.id}" data-action="trash">回收站</button>`}</div></td></tr>`).join('')}</tbody></table></div>`:empty(section==='trash'?'回收站是空的':'暂无匹配文件','files');html+=pagination(d.total);if(section==='files')html='<div id="fileWorkspace">'+html+'</div>';
  ...
  $('content').onclick=async e=>{const b=e.target.closest('button');if(!b){const tile=e.target.closest('[data-tile]');if(tile&&!e.target.closest('input')){const box=tile.querySelector('[data-select]');box.checked=!box.checked;box.dispatchEvent(new Event('change',{bubbles:true}));}return;}try{
    ...
    if(b.dataset.file){const file=currentFiles.find(f=>f.id===b.dataset.file),action=b.dataset.action;
      ...
      if(action==='office-edit')return await office.open(file);
      if(action==='office-versions')return await office.showVersions(file);
      if(action==='pdf-edit')return await pdf.open(file);
      if(action==='pdf-versions')return await pdf.showVersions(file);
  ...
  function openFileMenu(id,x,y,anchor){
    ...
    menu.innerHTML=`<strong>${esc(f.filename)}</strong><dl><dt>大小</dt><dd>${size(f.size)}</dd><dt>上传时间</dt><dd>${date(f.created_at)}</dd><dt title="仅统计已保存的文档和 PDF 版本，草稿自动同步不计">最近保存版本</dt><dd>${f.last_edited_at?date(f.last_edited_at):'—'}</dd><dt>分享</dt><dd>${f.active_share_count?'分享中':'未分享'} · 累计 ${f.share_count} 次</dd><dt>访问 / 下载</dt><dd>${f.view_count} / ${f.download_count}</dd></dl><div class="context-actions"><button data-file="${f.id}" data-action="preview">${icon('eye')}预览</button>${office.canEdit(f)?`<button data-file="${f.id}" data-action="office-edit">${icon('file-pen-line')}在线编辑</button><button data-file="${f.id}" data-action="office-versions">${icon('history')}版本记录</button>`:''}${pdf.canEdit(f)?`<button data-file="${f.id}" data-action="pdf-edit">${icon('files')}整理 PDF 页面</button><button data-file="${f.id}" data-action="pdf-versions">${icon('history')}版本记录</button>`:''}<button data-single-share="${f.id}">${icon('link')}创建分享</button><button data-file="${f.id}" data-action="rename">${icon('pencil')}重命名</button><button data-file="${f.id}" data-action="trash" class="danger">${icon('trash-2')}移入回收站</button></div>`;
  ...
  async function refreshFiles(automatic){
    ...
    if(uploading||office.isOpen()||pdf.isOpen()||$('dialog').open||$('fileContextMenu')){if(!automatic)toast('请先完成当前上传或操作');return;}
