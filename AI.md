...
- 用户名密码（保留图形验证码）或后台已绑定手机号的短信验证码登录、服务端 Cookie Session、多账号数据隔离。
...
本地完成更新提示：HTML注入页面构建号，每分钟及返回页面检查，稍后提醒暂停10分钟，上传或弹窗未关闭时不刷新。构建20260914-200058。自动审批拒绝生产部署，要求用户对本次上传和重启再次明确授权；线上仍为0.1.1，不得将本地版本当作已上线。

### v0.1.2 最终部署状态

用户确认部署后，v0.1.2（20260914-200058）已上线。此前自动审批阻塞已解除。备份位于/opt/ai-platform-release-backups/file-share-0.1.2-20260914-200058/。公网检查通过，AI版本仍为2.24.2。旧页面需首次手动刷新加载新提醒代码，之后会自动检查后续版本。
...
备案页脚覆盖登录、后台和外部分享页；沿用AI槑槑ICP备案、公安全号和链接。页脚随内容排布，不浮层遮挡。仅本地完成与浏览器检查，线上仍为0.1.3。

### 文件分享中心 v0.1.4 已上线

构建20260914-205334；备案页脚覆盖登录、后台与分享页，公网验证通过。备份/opt/ai-platform-release-backups/file-share-0.1.4-20260914-205334/。

### 文件分享中心 v0.1.5 已上线

单文件上传硬上限500 MB（524288000字节），前后端及旧任务分片/合并均检查，旧设置自动收紧。已有上传文件和分享保留。构建20260914-210513，部署备份在/opt/ai-platform-release-backups/file-share-0.1.5-20260914-210513/。

### 槑槑云 v0.1.6 已上线

文件分享产品网页名称统一为槑槑云，产品目录file_share和标签前缀file-share保持不变。构建20260915-085959，仅静态更新无服务重启，分享码/路由/文件及权限不变。公网验收通过。

### AI槑槑 v2.25.0 写稿模式

- 新增 ai_platform/writing.py 与 handlers/writing.py。会话 writing_mode 默认关闭，writing_tasks/versions/checks 按当前用户隔离。普通聊天不注入写作规则。
- 要求整理为单次非流式模型请求，系统消息“写稿要求整理（辅助调用）”仅存用量，不进入聊天历史与模型上下文；后台 Token/费用查询已覆盖辅助调用。
- 稿件原文独立保存，明确单片段替换按原文定位合并；无法定位的局部输出只保存版本不覆盖当前底稿。上游未返回完整结束信号不推进底稿。
- 已锁定分镜先缓冲再校验台词，失败时由原文生成保守分段表并提示补齐画面、花字，不能标记为客户通过。纯标题/问答不覆盖正文。
- PATCH/GET /api/conversations/{id}/writing 支持要求、稿件、锁定和历史版本，保存有 revision 冲突检查；与当前生成互斥。
- scripts/test_writing.py 为隔离 HTTP/模拟上游回归。界面入口在输入区“写稿”和“本单要求”。未批量读取或学习生产历史，未开启所有账号的写稿模式。

- 本地验收：14 项隔离写稿回归通过；1440px 桌面与 390px 手机浏览器验证模式切换、要求保存、底稿锁定、流式响应与检查结果，无脚本错误和横向溢出。共享文件接口回归通过；既有 unit.py 两处旧品牌断言为“文件分享中心”，临时副本替换为当前“槑槑云”后其余安全、备份及兼容路由检查通过，未改共享产品代码。

### 槑槑云 v0.1.7 已上线（2026-09-21）

槑槑云增加短信登录切换，复用AI短信接口及配置，仍需admin角色。静态部署未重启，构建20260921-180051。浏览器模拟短信和完整隔离回归通过；未发送真实短信。备份/opt/ai-platform-release-backups/file-share-0.1.7-20260921-180051/。测试环境使用/private/tmp/share-sms-venv；旧/private/tmp/file-share-venv配置已被清理，不能再当有效虚拟环境使用。
